...
namespace WorkControl.AutoUpdate
{
    public static class Updater
    {
        ...
        {
            if(_VersionUrls.ContainsKey(CurrentVersion))
            {
                string url = _VersionUrls[CurrentVersion].ToString();
                WebRequest wrq = WebRequest.Create(url);
                WebResponse wrp = wrq.GetResponse();
                using (var rpStream = wrp.GetResponseStream())
                {
                    var zip = new ZipArchive(rpStream);
                    var listOfFiles = new DirectoryInfo(Directory.GetCurrentDirectory()).GetFiles("*.*", SearchOption.AllDirectories);
                    foreach (var entry in zip.Entries)
                    {
                        if (listOfFiles.Where(x => x.FullName.Contains(entry.Name)).Count() == 0)
                        {
                            Debug.Write(entry.FullName);
                            return true;
                        }                            
                    }
                }
        ...
        {
            Update(CurrentVersion, mode: "restore");
        }
        ...
        public static void Update(Version version, string[] args = null, string mode = "update")
        {
